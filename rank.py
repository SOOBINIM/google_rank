...
# Mobile user agent https://deviceatlas.com/blog/mobile-browser-user-agent-strings
mobile_agent = [
    # Safari for iOS
...
# desktop user agent https://deviceatlas.com/blog/list-of-user-agent-strings
desktop_agent = [
    # Chrome 60 Windows
...
def desktop(keyword, sitename, device, useragent, searchList):
    parser = 'html.parser'
    ...
    for i in links:
        ...
        if sitename in str(i):
            ...
            d.append(device)
            d.append(now)
            print("{0}/{1}/{2}".format(keyword, position, now))
            print("d : " + d)
            searchList.append(d)
            print(searchList)
...
def csv_export(d, keyword, device):
    file = datetime.date.today().strftime("%d-%m-%Y")+'-' + \
        keyword + '-' + device + '.csv'
...
if device == 'mobile':
    ...
    for keyword in keywords['Keywords']:
        print(keyword)
        desktop(keyword, sitename, device, useragent, searchList)
        t = randint(1, 10)
        print('Sleeping time is', t, 'Seconds')
        time.sleep(t)
else:
